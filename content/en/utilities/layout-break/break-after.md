...
Модификатор `break-after` в SIMAI Framework позволяет контролировать разрывы столбцов или страниц после определённы
...
- Контрольная точка (необязательный параметр):  
  ...
  `xl`.  
  Если не указана, модификатор действует на все
 размера
.
...
- Модификатор (обязательный параметр):
    ...
    - `break-after-page` — обязательный разрыв и пере
од к новой странице после текущего элемента.
...
точки. Например, `md:break-after-column` заставит применять принудительный разрыв колонок только на экрана
 с размером
`Medium` и больше:
