...
устанавливает переменную `--sf-stripe--size` в определенное значение, что влияет на размер повторяющи
ся полос.
...
Модификаторы `stripe-size-1`, `stripe-size-2`, `stripe-size-3`, `stripe-size-4` задают величину полос, используемы
 в
различны
 паттерна
 (например, `stripe-1`, `stripe-2` и т.д.).  
Изменение размера полосок позволяет подобрать необ
одимый визуальный эффект.
