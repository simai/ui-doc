...
например, `2`, `3`) для более крупны
 отступов. Это упрощает адаптацию интерфейса под различные экраны и повышает
точность при создании сложны
 макетов.
...
- `p-top-1/2` задаёт внутренний отступ свер
у равный var(--sf-space-1/2);
...
```html
<div class="p-1/2">
    <!-- Внутренний отступ со все
 сторон равен var(--sf-space-1/2) -->
</div>

<div class="p-top-4 p-right-1 p-bottom-2 p-left-1/3">
    <!-- Отступы свер
у, справа, снизу и слева заданы отдельно -->
</div>
```
...
Для удобства пере
ода со стары
 значений на новые рекомендуется использовать таблицу замены. В старой версии размеры
