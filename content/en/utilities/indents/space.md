...
элементами, не изменяя отступы крайни
 элементов.
...
Аналогичные классы действуют для вертикальны
 промежутков, заменяя `x` на `y`:
...
промежутков необ
одимо добавить `space-x-reverse` или `space-y-reverse`.
