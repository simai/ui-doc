...
- Модификатор *(обязательный параметр)*:
    ...
    - `t-ellipsis` – текст обрезается с многоточием, необ
одимо вручную указать `overflow-hidden` и
      `white-space: nowrap`.
    - `t-clip` – текст обрезается без многоточия, необ
одимо вручную указать `overflow-hidden` и `white-space: nowrap`.
