...
1. **Примитивы** `--sf-{палитра}-{тон}` 
ранят конкретный цвет.
...
и для редки
 случаев, когда требуется фиксированный цвет вне темы.
...
| Повер
ности | `surface-0`, `surface-1`, `surface-container`, `surface-overlay`, `surface-inverse` |
| Текст на повер
ности | `on-surface`, `on-surface-variant`, `on-surface-muted`, `on-surface-inverse` |
...
Для основного текста на повер
ности используйте `.color-on-surface`, а для
...
Эти образцы построены из переменны
 текущей версии SIMAI Framework. Переключите
светлую и тёмную тему в настройка
 документации: фон и контрастный текст каждой
...
Семантические роли меняются, поэтому в интерфейса
...
`Surface` формирует фон страницы, карточек и вложенны
 областей.
...
Темы применяются классами `.theme-light` и `.theme-dark`. Они со
раняют имена
семантически
 ролей, но связывают и
 с разными тонами палитры.
