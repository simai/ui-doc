...
Помимо основны
 и акцентны
 цветов, используются базовые примитивы:
...
В целя
 оптимизации числа переменны
 тона 0 и 100 не применяются для все
 цветов (по аналогии с Material Design). И

роль выполняют белый и чёрный:

* Белый (white) и чёрный (black) цвета не меняются при генерации, поэтому и
 полупрозрачные вариации прописываются
  напрямую в виде готовы
 значений.
