...
Дизайн-токены связывают базовые [размерные примитивы](/ru/guide/fundamentals/size-scale/)
с назначением в интерфейсе. Их можно переопределять на уровне проекта, не
изменяя классы утилит.

## Когда применять

Используйте семантический токен, когда значение выражает устойчивую роль: интервал раздела, радиус карточки, размер иконки, тень или слой.
...
не номера шкалы напрямую, а [семантические токены вертикального ритма](/ru/guide/fundamentals/spacing/).

## Радиусы

Framework разделяет радиусы по назначению:

- `--sf-radius--ui` — системный радиус компактных элементов управления:
  кнопок, полей, выпадающих списков, чекбоксов и других небольших контролов;
- `--sf-radius-default` — системный радиус крупных поверхностей: карточек,
  панелей, самостоятельных блоков и контейнеров.

Компонент сам выбирает подходящий семантический токен. Утилиты
`radius-square` и `radius-rounded` применяйте только как явное исключение из
этого правила. Благодаря разделению зон можно независимо настроить геометрию
контролов и поверхностей для всего проекта.
...
```css
:root {
    --project-card-radius: var(--sf-radius-default);
    --project-card-gap: var(--sf-space-3);
}

.project-card {
    border-radius: var(--project-card-radius);
    padding: var(--project-card-gap);
}
```
...
ссылайтесь в них на глобальные значения. В примере роль карточки получила собственные имена проекта, но продолжает брать согласованные значения из Framework.
