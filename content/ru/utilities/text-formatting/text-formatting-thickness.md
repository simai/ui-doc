---
title: "Толщина оформления текста"
description: "Толщина оформления текста"
tags: [text-decoration-thickness]
---

# Толщина оформления текста

:badge[text-decoration-thickness]{type=main scheme=on-surface size=1}

## Наглядный пример

:::example {id="utilities/text-formatting/text-formatting-thickness" label="Толщина оформления текста"}
:::

## Таблица классов

| Класс | Значение |
|:--|:--|
| `decoration-auto` | Автоматическая толщина |
| `decoration-font` | Толщина из шрифта |
| `decoration-0 ... decoration-4` | Толщина по токенам `--sf-a0..--sf-a4` |

## Пример
