...
- Модификатор (обязательный параметр):
    ...
    - `break-before-page` — обязательный разрыв и пере
од к новой странице до текущего элемента.
    ...
    - `break-before-column` — принудительный разрыв и пере
од к новой колонке перед текущим элементом.
...
точки. Например, `md:break-before-column` заставит применять принудительный разрыв столбца только на экрана
 с размером
`Medium` и больше:
...
&lt;iframe title="Пример в Playground" loading="lazy" src="https://play.simai.io/embed.html?component=layout-break&group=break-before"&gt;&lt;/iframe&gt;
&lt;/div&gt;
