...
Модификаторы из пакета `scroll-padding` позволяют задать внутренние отступы при прокрутке, чтобы элементы при пере
оде к
...
помогает при пере
оде к элементу, чтобы он не был прижат к самому краю области просмотра, обеспечивая лучший обзор
содержимого.
