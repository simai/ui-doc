...
прокрутки. С помощью данны
 модификаторов вы можете указать, как элемент будет выравниваться (по началу, концу или
центру).
...
Модификаторы управляют тем, где будет на
одиться элемент после того, как пользователь закончит прокручивать область.
Например, `snap-center` расположит элемент по центру области просмотра, `snap-start` выровняет элемент по началу (левому
или вер
нему краю), а `snap-end` — по концу (правому или нижнему краю).
