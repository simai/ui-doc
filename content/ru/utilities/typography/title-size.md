...
:badge[title]{type=main scheme=on-surface size=1} :badge[headers]{type=tonal scheme=neutral size=1}

Пакет `headers` задаёт согласованные базовые размеры элементам `h1`–`h6`.
Классы `title-*` позволяют применить размер заголовка к любому элементу.

Набор модификаторов для заголовков использует различные наборы переменных для размера шрифта и для высоты
...
```html
<p class="title-2">На экранах md и больше будет применён title-2</p>
```
