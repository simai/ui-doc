...
- Классы одинаково работают в LTR и RTL и не меняют содержимое текста.
- Responsive- и state-варианты, отрицательные значения и публичные псевдонимы
  для семейства `text-wrap` не выпускаются.
- `text-balance` и `text-pretty` зависят от поддержки браузером. Там, где режим
  не поддержан, сохраняется обычное поведение переноса текста.
