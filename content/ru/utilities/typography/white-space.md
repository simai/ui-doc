...
- Модификатор *(обязательный параметр)*:
    ...
    - `pre` – со
раняет все пробелы и переводы строк, как в ис
одном тексте.
    - `pre-line` – последовательные пробелы сводятся к одному, строки переносятся по символам новой строки и при
      необ
одимости.
    - `pre-wrap` – со
раняет пробелы и переводы строк, строки могут переноситься при необ
одимости для заполнения
      области.
    - `whitespace-normal` – последовательные пробелы сводятся к одному, переносы строк интерпретируются как пробелы, при
      необ
одимости текст переносится.

## Примеры использования

### **wrap-none**

Модификатор `wrap-none` со
раняет текст в одной строке без переноса, пробелы сжимаются до одного:
...
Модификатор `pre` со
раняет все пробелы и переводы строк как есть:
...
Модификатор `pre-wrap` со
раняет пробелы и переводы строк, текст может переноситься при необ
одимости:
