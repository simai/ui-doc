---
title: "Offset"
description: "Точный Loader- и CSS-контракт утилиты offset в SIMAI Framework 5.4.0 candidate."
---

# Offset

Идентификатор: `utility.offset`. Статус готовности: **доступен, но ещё не прошёл полную продуктовую приёмку**.

## Loader-контракт

- `offset/default`
- `offset/lg`
- `offset/md`
- `offset/sm`
- `offset/xl`

Корень ассетов: `distr/utility/offset`.

## Поставляемые классы

Всего уникальных классов в readable CSS: **58**.

```text
333333
666667
lg:offset-1/12
lg:offset-10/12
lg:offset-11/12
lg:offset-2/12
lg:offset-3/12
lg:offset-4/12
lg:offset-5/12
lg:offset-6/12
lg:offset-7/12
lg:offset-8/12
lg:offset-9/12
md:offset-1/12
md:offset-10/12
md:offset-11/12
md:offset-2/12
md:offset-3/12
md:offset-4/12
md:offset-5/12
md:offset-6/12
md:offset-7/12
md:offset-8/12
md:offset-9/12
offset-1/12
offset-10/12
offset-11/12
offset-2/12
offset-3/12
offset-4/12
offset-5/12
offset-6/12
offset-7/12
offset-8/12
offset-9/12
sm:offset-1/12
sm:offset-10/12
sm:offset-11/12
sm:offset-2/12
sm:offset-3/12
sm:offset-4/12
sm:offset-5/12
sm:offset-6/12
sm:offset-7/12
sm:offset-8/12
sm:offset-9/12
utilities
xl:offset-1/12
xl:offset-10/12
xl:offset-11/12
xl:offset-2/12
xl:offset-3/12
xl:offset-4/12
xl:offset-5/12
xl:offset-6/12
xl:offset-7/12
xl:offset-8/12
xl:offset-9/12
```

## Источник

- `simai/ui@2742ed22730b3f37cd26ab72c03621637a464ee0:distr/rule/rule.json#family=offset`
- `simai/ui@2742ed22730b3f37cd26ab72c03621637a464ee0:distr/utility/offset`
