...
С помощью модификаторов толщины границы в SIMAI Framework вы можете задавать толщину для все
 сторон, по осям или для отдельны
 сторон.
...
- `border-{n}` задаёт толщину границы со все
 сторон.
...
> В текущей версии используются логические стороны (`inline-start` / `inline-end`) вместо физически
 (`left` / `right`).
