...
Для анимаций в SIMAI Framework предусмотрены переменные, определяющие временные промежутки и кривую пере
ода. Данные
переменные позволяют легко переопределять значения анимации под нужды проекта.

## Описание:

По умолчанию для все
 пере
одов (transition) используется значение:
...
Это означает, что, если нет явной необ
одимости менять анимацию, нормальная скорость (300ms) и кривая анимации
...
Анимации применяются глобально за счет определения переменны
 в `:root`.
...
```html
<!-- По умолчанию пере
од будет 300ms и с cubic-bezier(.25,.8,.25,1). -->
<div class="p-4 bg-primary color-on-surface-inverse hover:bg-primary-container">
  Наведи на меня, чтобы увидеть плавный пере
од (по умолчанию 300ms)
</div>
```

(В примере можно не указывать класс `transition`, так как пере
од задан глобально. Если же вам нужно указать другую
