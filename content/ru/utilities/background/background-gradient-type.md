...
Утилиты `gr-*` задают тип градиента и то, сколько цветовы
 точек используется. Сами цвета и угол задаются через отдельные утилиты:
...
- Угол (для `gr-line-*`): через CSS‑переменную `--sf-gradient--angle` (по умолчанию 90deg), при необ
одимости переопределяется инлайном или кастомным классом.
...
**Важно:** без указания цветов (`gr1-/gr2-/gr3-` или `from-/via-/to-`) градиент будет прозрачным, так как в переменны
 `--sf-gradient--color-*` нет значений.
## Playground
