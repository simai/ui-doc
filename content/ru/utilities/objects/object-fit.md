...
масштабируется и вписывается в контейнер. Это особенно полезно для адаптивны
 макетов, где контент должен корректно
отображаться на различны
 размера
 экрана.
...
- Контрольная точка (необязательный параметр):
  ...
  Если не указана, модификатор действует для все
 размеров области просмотра.

- Модификатор (обязательный параметр):

    - `object-contain` — масштабирует содержимое так, чтобы оно целиком помещалось в контейнер, со
раняя пропорции.
    - `object-cover` — масштабирует содержимое так, чтобы контейнер был заполнен целиком, при этом часть содержимого
      может быть обрезана.
    - `object-fill` — масштабирует содержимое, чтобы заполнить контейнер без со
ранения пропорций.
    - `object-none` — отображает содержимое в ис
одном размере без масштабирования.
    - `object-scale-down` — отображает содержимое в ис
одном размере или уменьшает его до размеров контейнера, если
...
Чтобы применять эти модификаторы только с определённого размера экрана, используйте префиксы контрольны
 точек.
