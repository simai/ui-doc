# Таблицы

Утилиты раздела помогают управлять отображением таблиц: базовым стилем, компактностью, границами, чередованием строк/столбцов, активным состоянием и алгоритмом раскладки.

## Материалы раздела
- [Параметры по умолчанию для таблиц](/ru/utilities/tables/tables-default-parameters/)
- [Оформление таблицы](/ru/utilities/tables/tables-formatting/)
- [Чередование строк и столбцов](/ru/utilities/tables/tables-alternating-rows-and-columns/)
- [Активные строки и ячейки](/ru/utilities/tables/tables-active-rows-and-cells/)
- [Границы таблицы](/ru/utilities/tables/table-border/)
- [Границы вокруг ячеек](/ru/utilities/tables/table-border-cells/)
- [Макет таблицы](/ru/utilities/tables/table-layout/)
- [Границы и интервалы таблицы](/ru/utilities/tables/table-border-spacing/)
