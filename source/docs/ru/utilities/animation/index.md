# Анимация

Утилиты категории Анимация помогают настраивать связанные CSS-свойства и паттерны в интерфейсе.

## Материалы раздела
- [Параметры по умолчанию для анимации](animation-default-parameters.md)
- [Продолжительность анимации](animation-duration.md)
- [Анимация при появлении](animation-transition-appearing.md)
- [Задержка перехода](animation-transition-delay.md)
- [Продолжительность перехода](animation-transition-duration.md)
- [Общее свойство перехода](animation-transition-property.md)
- [Функция времени перехода](animation-transition-timing-function.md)
- [Тип анимации](animation-type.md)

## Связанные категории
- [Интерактивность](../interactivity/index.md)
- [Преобразования](../transform/index.md)
- [Фильтры элемента](../filters/index.md)
