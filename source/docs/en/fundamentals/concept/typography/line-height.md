...
In the new version, line height is calculated separately for body text and headings, taking into account the 0.25rem
(4px) step. This helps maintain consistency in design.

**General rules:**

* **Body text:**  
  Line height РІвЂ°в‚¬ 1.5 of the font size. If the result is not a multiple of 0.25rem, it is decreased to the nearest valid
  value.
* **Headings:**  
  Line height РІвЂ°в‚¬ 1.2 of the font size. The minimum heading height is not less than 1.  
  If the result is not a multiple of 0.25rem, the value is also rounded down to the nearest valid value.

If the final number does not fall into the available value range, use the nearest lower value.

## Line height table for **body text**
...
For headings, the `title` prefix is used in variable names. This makes it easier to distinguish them from base text and
accounts for the reverse order of sizes (heading 1 corresponds to height 6).
...
To switch line height from regular text to heading, use the `.heading` modifier. It replaces the line height of body
text with the line height of a heading. Connect these styles after defining the height for body text.
...
For the **label** role, the following line-height variables are used:
