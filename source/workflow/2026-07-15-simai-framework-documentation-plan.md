...
Статус пакета от 2026-07-24:

- в меню выведены размеры, параметры модификаторов, сокращения, направления и
  выравнивание;
- добавлены полная шкала из 90 размерных примитивов, все восемь цветовых
  палитр, семантические роли тем и сводный справочник дизайн-токенов;
- актуализированы breakpoints, состояния и адаптивные шкалы типографики;
- production build и 75 570 внутренних ссылок проходят без битых маршрутов;
- legacy-дубли `concept`, `color`, `typography` и `variables` пока сохранены
  скрытыми: их очистка остаётся отдельным migration batch.
